<template>
  <div>
    ...
    <v-data-table :headers="headers" :items="jenkinsData" hide-default-footer>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-container>
            ...
            ></v-text-field>
            <v-text-field
              v-model="jenkinsUsername"
              label="jenkins用户名"
            ></v-text-field>
            <v-text-field
              v-model="jenkinsPassword"
              type="password"
              label="jenkins密码"
            ></v-text-field>
            <v-select
              v-model="commandRunCaseType"
              :items="runType"
              label="测试用例类型"
            ></v-select>
            <v-text-field
              v-model="commandRunCaseSuffix"
              label="测试用例后缀名"
            ></v-text-field>
...
export default {
  ...
  methods: {
    ...
    saveJenkins(params) {
      ...
      this.$api.project.addJenkins(post_data).then((res) => {
        console.log(res)
        this.close()
        this.getJenkinsList()
      })
